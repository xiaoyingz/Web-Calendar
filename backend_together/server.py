...
import daily_summary_database, daily_summary_schema, summary_query_parser, task_database
import time
...
def get_task_by_id():
    ...
    if task_id:
        find_result = daily_summary_database.find_summary_by_id(task_id, collection_name='task')
        if len(find_result) == 0:
...
@app.route('/task/pie', methods=['GET'])
def get_type_count():
    """
    Define route getting count for each type of tasks, i.e. "/task/pie?date=2021-04-23"
    :return: json with type of task as keys and corresponding count as values, or error message.
    """

    task_date = request.args.get('date', None)
    if task_date is None:
        return jsonify(INVALID_REQUEST.format('task')), 400

    find_result = task_database.find_task_by_date(task_date, 0)
    if len(find_result) == 0:
        return jsonify(NOT_FOUND_MESSAGE.format('task')), 400
    
    count = create_count(find_result)
    return jsonify(count), 200


def create_count(find_result):
    result = {}
    total = len(find_result)
    for doc in find_result:
        curr_type = doc['type']
        result[curr_type] = result.get(curr_type, 0) + 1
