...
OP_ESCAPE = ['\:', '\$', '\AND', '\OR', '\NOT', '\BEFORE', '\AFTER', '\BETWEEN', '\"']
OP_MASK = ['C_O_L', 'D_O_L', 'A_N_D', 'O_R', 'N_O_T', 'B_E_F', 'A_F_T', 'B_E_T', 'Q_U_O_T_E']
INT_ATTR = ['year', 'month', 'day']

SINGLE_ATTR_QUERY = '{attr}: {value}'
...
def parser(query):
    ...
    attr = attr.strip()
    rule = rule.strip()
    if attr not in daily_summary_schema.summary_schema:
        return False, INVALID_ATTRIBUTE.format(attr)

    if attr in INT_ATTR:
        try:
            value = int(rule)
            str_q = '{' + SINGLE_ATTR_QUERY.format(attr=attr, value=value) + '}'
            result = call_db(str_q)
            return True, result
        except ValueError:
...
def call_db(str_query):
    curr_db = daily_summary_database.connect_db()
    mongo_q = json.loads(str_query)
...
def preprocess_query(query):
    for i in range(0, len(OP_ESCAPE)):
        query.replace(OP_ESCAPE[i], OP_MASK[i])


def revert_rule(rule):
    for i in range(0, len(OP_ESCAPE)):
        rule.replace(OP_MASK[i], OP_ESCAPE[i][1:])