...
export default class DailyView extends Component {
    ...
    constructor(props) {
        ...
        this.onClickDelayAll = this.onClickDelayAll.bind(this);
    ...
    async getLuckyNumber() {
        const response = await TaskService.getLuckyNumber();

        let day = new Date();
        let today = format(day, "yyyy-MM-dd");
        let data = response.data
        if (today != this.state.date) {
            data = "Oops, no lucky number for the past or the future"
        }
        this.setState({
            luckyNumber: data
        });
    ...
    async onClickDelayAll() {

        try {
            const { date } = this.state;
            const response = await TaskService.delayTasksByDate(date);
            this.setState({
                delayMessage: 'Successfully delay all the unfinished tasks',
            });
            this.retrieveTasks(date);
            ...
            console.log(err);
            this.setState({
                delayMessage: 'Failed to delay all the unfinished tasks.',
            });
        }
        ...
        return (
            <div className="list row">
                <div style={{ marginBottom: '1rem' }}>
                    <h3>{date}</h3>
                    <div>Lucky Number: {this.state.luckyNumber}</div>
                ...
                <div className="list row">
                    <div className="col-md-6" style={{ marginBottom: '2rem' }}>
                        <div>
                            ...
                            <button type="button" onClick={this.onClickDelayAll} className="badge badge-info mr-2">
                                        Delay All
                                    </button>
                        </div>
                        {taskMessage === '' ? (
